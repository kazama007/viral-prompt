...
function formatUser(u) {
  ...
  const isAdmin = cleanEmail === 'sa812sn@gmail.com';

  const evaluatedSub = isAdmin ? {
    status: 'active',
    plan: 'admin_lifetime',
    grantedAt: u.created_at || '2026-09-06T00:00:00.000Z',
    expiresAt: null,
    durationDays: 99999,
    isActive: true,
    isExpired: false,
    ...
    grantedAtFormatted: formatDateDayMonthYear(u.created_at || '2026-09-06T00:00:00.000Z')
  } : evaluateSubscription(rawSub);
...
async function getCloudUserProfile(token) {
  ...
  try {
    ...
    // Attempt 2: If token is a JWT (has 3 parts), decode payload even if expired
    if (typeof token === 'string' && token.includes('.')) {
      try {
        ...
        if (parts.length === 3) {
          ...
          const jwtSub = payload.sub || payload.id;
          if (jwtSub && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(jwtSub)) {
            const adminUserRes = await supabase.auth.admin.getUserById(jwtSub);
          ...
          if (payload.email) {
            ...
            const match = (listData?.users || []).find(u => u.email?.toLowerCase().trim() === payload.email.toLowerCase().trim());
            if (match) return formatUser(match);
          }
        }
      } catch (jwtErr) {}
    }

    // Attempt 3: Direct email address passed
    if (typeof token === 'string' && token.includes('@')) {
      const cleanEmail = token.toLowerCase().trim();
      const { data: listData } = await supabase.auth.admin.listUsers({ perPage: 1000 });
      const match = (listData?.users || []).find(u => u.email?.toLowerCase().trim() === cleanEmail);
      if (match) return formatUser(match);
    }
